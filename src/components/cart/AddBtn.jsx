...
const AddBtn = ({ id, amount, stock_quantity }) => {
  const { increase, decrease, showAlert } = useGlobalContext();

  return (
    <>
      <div className="absolute bottom-0 rounded-[4px] bg-[#FFe141] w-full flex justify-between items-center border border-solid border-[#dddddd]">
        <div
          onClick={() => decrease(id)}
